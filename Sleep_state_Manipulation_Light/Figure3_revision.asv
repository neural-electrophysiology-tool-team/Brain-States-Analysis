...
f=figure;
x=1:3;
swarmchart(groupNum,stimP2V,20,colorMat,'filled','XJitterWidth',0.5);
hold on; scatter(1:type,means,'k','Marker','+')
xticks(1:type);xticklabels(stimWaveL)
...
%Plot
f=figure;
swarmchart(groupNum,DBdiff,20,colorMat,'filled','XJitterWidth',0.5);
hold on;
scatter(1:numType, DBMean,14,'k','Marker','+')
ylabel('d/b diff')
xticks(1:numType); xticklabels(stimWaveL)
...
for type = 1:numType
    ...
    headSDdiffM = [headSDdiffM; mean(curData,'omitnan')];
    groupNum = [groupNum; repmat(type,length(curData),1)];
    colorMat = [colorMat; curColorMat];
...
for i = 1:length(couples)
    for j = 1:2
        ...
        curColorMat = animalsColors(animalIndices, :);
        colorMat = [colorMat; curColorMat];
        pfrom156 = signrank(curDataAC,156000);
        psFrom156 = [psFrom156,pfrom156];
    end
    %statistics
    %diff between groups:
    curCoupleNum = [(i*2-1),(i*2)];
    ...
    fprintf('whiteCouple: p-value = %.4f', corrected_pvals_bonferroni(2));
    fprintf('\n ns:');fprintf('%d,',ns); fprintf('\n Ns:');fprintf('%d,',Ns); fprintf('\n')
    fprintf('diff from 156s:');fprintf('%.4f,',psFrom156);fprintf('\n');
%plot:
f=figure;
...
function [corrected_pvals] = plotRelativeTransmitionAnimal(results, filterLabels, generalFolder,colors, whichFilt)
    ...
    %which filter:
    if nargin ==5
        validInd2 = ismember(results.Filter,whichFilt);
        results = results(validInd2, :);
    end
    ...
    if pKW < 0.05
        ...
        for i = 1:numGroups-1
            for j = i+1:numGroups
                ...
                % Store results
                comparisons{idx,1} = [filterLabels{i} ' vs ' filterLabels{j}];
                raw_pvals(idx,1) = p;
                idx = idx + 1;
            end
        end
    ...
    % Save image
    outputFile = fullfile(generalFolder, 'TransmissionAnimal.pdf');
