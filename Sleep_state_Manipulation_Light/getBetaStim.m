...
function data = getBetaStim(SA, overwrite)
    ...
    SA.files.betaStim=[SA.currentAnalysisFolder filesep 'betaStim.mat'];
    if exist(SA.files.betaStim,'file') & ~overwrite
        if nargout==1
            data=load(SA.files.betaStim);
        else
...
%     clear StimDB; %change to zeros
    ...
    stimBeta = zeros(numel(firstTrig),win);
    for i=1:numel(firstTrig)
        ...
        stimBeta(i,:)=DB.bufferedBetaRatio(pTmp);
    end
...
%     end
   ...
   % save the data
save(SA.files.betaStim,'ts','stimBeta','stimDuration','pre','post')
% data.StimDBSham = StimDBSham;
data.ts = ts;
data.stimBeta = stimBeta;
