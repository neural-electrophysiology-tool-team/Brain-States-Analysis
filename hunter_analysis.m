...
if contains(vidName,'top')
    ...
    vidName = strrep(vidName,'top','back');
...
b=1;
    for u=1:length(ArenaCSVs.strikesFrame) %trigger_frame = frame of e.g. bug appearance - a few frames to check video before bug should appear
        for i=0:30% or
            ...
            if get(gcf,'CurrentCharacter')=='b' %if you want to endends the loop with q
                b=5;
                break
            end 
        end
...
function arenaCSVs = getArenaCSVs(SA,overwrite)
    ...
    SA.files.arenaCSV=[SA.currentAnalysisFolder filesep 'ArenaCSV.mat'];
    if exist(SA.files.arenaCSV,'file') & ~overwrite
        if nargout==1
            load(SA.files.arenaCSV);
    ...
    sTrialFrame = getVideoFrames(videoFrames,startTrials);
    eTrialFrame = getVideoFrames(videoFrames,endTrials);
    ...
    save(SA.files.arenaCSV,"arenaCSVs");
