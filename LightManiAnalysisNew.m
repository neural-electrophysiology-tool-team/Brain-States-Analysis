...
for type = 1:numType
    ...
    statsAll.(curName).alpha = alpha;
    statsAll.(curName).pAnova = p;
    statsAll.(curName).p_wake_pre = p_wake_pre;
    statsAll.(curName).p_wake_during = p_wake_during;
    statsAll.(curName).p_wake_after = p_wake_after;
    statsAll.(curName).p_pre_during = p_pre_during;
    statsAll.(curName).p_during_after = p_during_after;
    statsAll.(curName).p_pre_after = p_pre_after;
...
hP{1}=polarplot([relativePhasePre(p)';relativePhasePre(p)'],[zeros(1,numel(p)); ...
        Rlim*ones(1,numel(p))],'color',plotColors{1},'LineWidth',1);
hP{1}=polarplot([relativePreMean relativePreMean],[0,Rlim],'color',plotColors{1},'LineWidth',3);
    hP{2}=polarplot([relativePhaseStim(p)';relativePhaseStim(p)'],[zeros(1,numel(p));Rlim*ones(1,numel(p))],'color',plotColors{2},'LineWidth',1);
hP{2}=polarplot([relativeStimMean relativeStimMean],[0,Rlim],'color',plotColors{2},'LineWidth',3);
    hP{3}=polarplot([relativePhasePost(p)';relativePhasePost(p)'],[zeros(1,numel(p));Rlim*ones(1,numel(p))],'color',plotColors{3},'LineWidth',1);
